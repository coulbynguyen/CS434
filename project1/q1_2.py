...
#the set is split by spaces and represented as a list
#then the array adds a dummy variable to the front of the list
#then the last item in the list gets removed, since that item is the label
#the label is added to the list of labels that correlate to the the training features
#then the dummy variable and the rest of the features are added to a 2 dimensional matrix that contains all the training data
...
#this line transforms the training data matrix into an np.ndarry in order to perform matrix operations such as transpose and inverse
#the next line creates the variable that is the transpose of the training data matrix
...
w = w.tolist()
#this prints out the weight vector
print(w)
...
for x,y in zip(testingdataY, testingdatapredictY):
    ...
    diff = diff**2
    ASE += diff
#this prints out the ASE for the testing data
